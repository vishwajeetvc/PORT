...
function ProjectDes({project}){
    return <div className="lg:w-[60%] lg:pr-10 px-4 pb-5">
        <h2 className="text-4xl lg:text-5xl font-bold my-6 ">{project.name} -</h2>
        <div className="flex flex-wrap gap-3 lg:py-2">
            {project.technologys.map(item => <span key={item} className="py-1 px-2 text-sm font-bold rounded-full bg-gray-400/30 inline-block">{item}</span>)}
        </div>
        ...
        <div className="pt-5 flex gap-5 items-center">
            <a className="bg-orange-400/60 px-4 py-2" href={project.githubLink}>Github</a>
             <a className="border-b-1 px-4 py-1" href={project.githubLink}>Live Demo <span className="text-xl text-white/80 inline-block rotate-[-40deg]">➔</span></a>
        </div>
    </div>
}
